...
class PCLM_Visibility {
	...
	public static function init() {
		...
		add_action( 'pclm_menu_published', array( __CLASS__, 'flush' ) );
	}
	...
	public static function hidden_product_ids() {
		...
		foreach ( PCLM_Product::all_managed_ids() as $product_id ) {
			$opened_at = PCLM_Product::get_opened_at( $product_id );

			if ( ! $opened_at || ! PCLM_Schedule::is_listed( $opened_at ) ) {
	...
	/**
	 * The core of the cutoff: only a dish inside its window can be bought.
	 */
	public static function filter_is_purchasable( $purchasable, $product ) {
		...
		return PCLM_Schedule::is_orderable( $opened_at );
	}
	...
	/**
	 * Excludes expired dishes from every product listing, whichever plugin built
	 * the query. Page-builder grids run their own WP_Query, so filtering here
	 * rather than in WooCommerce's product query catches those too.
	 */
	public static function filter_queries( $query ) {
		...
		if ( $query->get( 'pclm_bypass_visibility' ) ) {
			return;
		}
	...
	/**
	 * Closes the direct-link hole: a bookmark to a dish from a finished week
	 * goes to the menu rather than a buyable page.
	 */
	public static function redirect_expired() {
		...
		if ( ! $product_id || ! PCLM_Product::is_managed( $product_id ) ) {
			return;
		}

		$opened_at = PCLM_Product::get_opened_at( $product_id );

		if ( $opened_at && PCLM_Schedule::PAST !== PCLM_Schedule::state_for( $opened_at ) ) {
			return;
		}
	...
	public static function menu_url() {
		$page_id = (int) PCLM_Settings::get( 'menu_page_id' );
	...
	public static function render_state_notice() {
		...
		if ( ! $opened_at || PCLM_Schedule::is_orderable( $opened_at ) ) {
			return;
		}

		printf(
			'<p class="pclm-state-notice">%s</p>',
